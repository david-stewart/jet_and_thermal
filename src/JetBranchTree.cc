...
void JetBranch::fill(fastjet::PseudoJet& jet) {
    pt = jet.perp();
    phi = jet.phi();
    eta = jet.eta();
    if (jet.has_area) area = jet.area();
    if (jet.has_constituents()) {
        if (!fill_area) {
            auto constituents = jet.constituents();
            nconsts = constituents.size();
            if (fill_constituents) {
                charge = 0;
                for (auto& C : constituents) charge += (int) C.user_index();
            }
        }
    }
}
