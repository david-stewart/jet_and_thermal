...
int main(int nargs, char** argv) {
  ...
  int pyth_seed         { (nargs>2) ? atoi(argv[2]) : 1 }; // -1 is default for Pythia
  string f_outstem      { (nargs>4) ? argv[4] : "pithy" };
  int bkg_seed { (nargs>3) ? atoi(argv[3]) :  -100 }; //  - is default for TRandom3
                                                                                       //
  ...
  for (int nev=0;nev<n_events;++nev) {
    ...
    if (print_Pyth_and_Bkg) { 
      ...
      for (auto& jet : jets_PB) {
        ...
        // Now recluster the jet and how what it is
        int ica {0};
    ...
    // recluster the jets with CA and follow the jet history
    ...
    if (print_matched) { 
      ...
      for (auto im : i_matcher.i_matched) {
        ...
        cout << Form("PJet[%2i] (%5.2f,%5.2f,pT:%5.2f) PBJet[%2i] (%5.2f,%5.2f,pT:%5.2f) Delta(%6.3f,%6.3f,pT:%6.2f,R:%6.3f(w/rap) or %6.3f(w/eta))",
